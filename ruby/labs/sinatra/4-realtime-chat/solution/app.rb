...
set :bind, '0.0.0.0'
enable :sessions
set :session_secret, ENV['SESSION_SECRET'] || 'chat-app-secret-key-change-in-production-this-must-be-at-least-64-bytes-long-for-security'
...
get '/rooms/:id' do
  ...
  @messages = Message.where(room_id: @room.id).order(:created_at).limit(50).all
